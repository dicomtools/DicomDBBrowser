function initBrowserDoubleClick(sApplication)
%function initBrowserDoubleClick(sApplication)
%Set Double Click Default Application.
...
% along with DicomDBBrowser.  If not, see <http://www.gnu.org/licenses/>.
    ...
    for mm=1: numel(asList)
        if strcmpi(sApplication, asList(mm) )
            browserDoubleClickOffset('set', mm);                
        end
    end                 
end
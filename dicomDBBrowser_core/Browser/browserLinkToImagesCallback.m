function browserLinkToImagesCallback(~, ~)
%function browserLinkToImagesCallback() 
%Call File Manager from ui Menu.
...
%                   
                    ...
                    try
                        winopen(char(sPath)); % Windows
                    catch
                        try
                            system(['nautilus ' char(sPath) '&']); % Linux
                        catch
                            system(['open ' char(sPath) '&']); % MAC
                        end
                    end
                end

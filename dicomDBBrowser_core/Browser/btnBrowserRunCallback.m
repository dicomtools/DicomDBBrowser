function btnBrowserRunCallback(hObject, ~)
%function btnBrowserRunCallback(hObject, ~)
%Browser Run Application Call from ui Menu.
...
% along with dicomDBBrowser.  If not, see <http://www.gnu.org/licenses/>.
    ...
    for ll=1:numel(atAppList)
        if strcmp(hObject.String, atAppList(ll).sDisplayName)
            set( btnBrowserRunCommandPtr('get'), 'Value', ll );
            set( btnBrowserRunCommandPtr('get'), 'String', atAppList(ll).sDisplayName );

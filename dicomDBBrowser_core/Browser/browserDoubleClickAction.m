function browserDoubleClickAction(sAppName)
%function browserDoubleClickAction(sAppName)
%Browser Main Window Double Click Action.
...
% along with dicomDBBrowser.  If not, see <http://www.gnu.org/licenses/>.
    ...
    elseif strcmpi(sAppName, 'CERR')   
        ...
        browserCerrCallbak();        
    elseif strcmpi(sAppName, 'Open Containing Folder')   

        browserLinkToImagesCallback();
    elseif strcmpi(sAppName, 'Export Anonymize')   

        browserDicomAnonymizerCallback();
    else
        atAppList = browserRunAppList('get');
        
        for kk=1:numel(atAppList)
            if strcmpi(sAppName, atAppList(kk).sDisplayName) 

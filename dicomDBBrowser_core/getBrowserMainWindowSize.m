function lSize = getBrowserMainWindowSize(sType)
%function lPosition = getBrowserMainWindowSize(sType)
%Get Main Window X or Y Size.
...
% along with dicomDBBrowser.  If not, see <http://www.gnu.org/licenses/>.
    ...
    alPosition = get(dlgBrowserWindowsPtr('get'), 'Position');

    if strcmp('xsize', sType)

        lSize = alPosition(3) * dScreenSize(3);
     else

        lSize = alPosition(4) * dScreenSize(4); 
     end    
end
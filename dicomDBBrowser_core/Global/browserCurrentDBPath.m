function sCurrentDBPath = browserCurrentDBPath(sAction, sValue)
%function sCurrentDBPath = browserCurrentDBPath(sAction, sValue)
%Get/Set Browser Current Database Path.
...
% along with DicomDBBrowser.  If not, see <http://www.gnu.org/licenses/>.
    ...
    persistent psCurrentDBPath;        

    if strcmp('set', sAction)
        psCurrentDBPath = sValue;
    end

    sCurrentDBPath = psCurrentDBPath;
end 
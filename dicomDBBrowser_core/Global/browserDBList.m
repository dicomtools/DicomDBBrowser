function atList = browserDBList(sAction, tValue)
%function atList = browserDBList(sAction, tValue)
%Get/Add/Clear Browser Database List.
...
% along with dicomDBBrowser.  If not, see <http://www.gnu.org/licenses/>.
    ...
    if strcmp('add', sAction)
        patList{numel(patList)+1} = tValue;
    elseif strcmp('clear', sAction)
        clear patList; 
    else
        atList = patList;
    end

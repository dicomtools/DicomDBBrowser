function lPosition = getBrowserMainWindowPosition(sType)
%function lPosition = getBrowserMainWindowPosition(sType)
%Get Main Window X or Y Position.
...
% along with dicomDBBrowser.  If not, see <http://www.gnu.org/licenses/>.
    ...
    if strcmp('xpos', sType)
        lPosition = alPosition(1) * dScreenSize(3);
    else
        lPosition = alPosition(2) * dScreenSize(4); 
    end    
end